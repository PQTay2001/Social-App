...
export default function UpdateProfile(props) {
  ...
  const handleGallery = () => {
    ...
    const updatedetails=async()=>{
      ...
      {
        alert("Một số trường chưa được điền")
        return
      }
      try{
      showindicator(true)  
  ...
  if(indicator)
  {
    ...
    <Screen style={{ flex: 1,backgroundColor: colors.white }}>  
        ...
        <Dialog.Title>Tùy chọn hình ảnh</Dialog.Title>
        <Dialog.Description>
          Chọn hình ảnh từ thiết bị hoặc trực tiếp từ máy ảnh
        </Dialog.Description>
        <Dialog.Button label="Máy ảnh" onPress={handleCamera} />
        <Dialog.Button label="Thiết bị" onPress={handleGallery} />
      </Dialog.Container>
    ...
    <View style={{padding:RFPercentage(2)}}>
        ...
        <Ionicons name="menu-outline" size={35} color={colors.mblack} />
            ...
            <Text style={{fontSize:RFPercentage(2.5)}}>Cập nhật hồ sơ</Text>
            <TouchableOpacity>
...
<TextInput style={{borderColor:colors.grey,borderWidth:1,color:colors.mblack,height:RFPercentage(7),width:"90%",borderRadius:RFPercentage(1),padding:RFPercentage(2),marginVertical:RFPercentage(2)}} placeholder='Tên tài khoản' onChangeText={(text)=>setusername(text)} value={username} />
<TextInput style={{borderColor:colors.grey,borderWidth:1,color:colors.mblack,height:RFPercentage(7),width:"90%",borderRadius:RFPercentage(1),padding:RFPercentage(2)}} placeholder='Thông tin' onChangeText={(text)=>setbio(text)} value={bio} />
