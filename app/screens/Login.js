    ...
    const handellogin=()=>{
        ...
        try{
            ...
            signInWithEmailAndPassword(auth,tempfeilds[0].value,tempfeilds[1].value).then(()=>{
                showindicator(false);
                let toast = Toast.show('Đăng nhập thành công', {
                    duration: Toast.durations.LONG,
                  });
                  setTimeout(function hideToast() {
                    Toast.hide(toast);
            ...
            showindicator(false);
            let toast = Toast.show('Đăng nhập thất bại', {
                duration: Toast.durations.LONG,
              });
              setTimeout(function hideToast() {
                Toast.hide(toast);
    ...
    return (
        ...
        <LoadingModal show={indicator}></LoadingModal>
            ...
            <Text style={{color:colors.mblack,marginVertical:RFPercentage(2), textAlign:"center"}}>
                Nhập Email và mật khẩu để đăng nhập ứng dụng
            </Text>
            {
            ...
            <TouchableOpacity onPress={()=>props.navigation.navigate("ResetScreen")} style={{marginVertical:RFPercentage(10)}} >
                <Text style={{color:colors.pink,textAlign:"center"}}>Quên mật khẩu? </Text>
            </TouchableOpacity>
            </View>
